...
TextGameObject::TextGameObject(const glm::vec3 &position, Geometry *geom, Shader *shader, GLuint texture, int type) 
: GameObject(position, geom, shader, texture, NULL, type, NULL) {
    text_ = "";
}
...
void TextGameObject::Render(glm::mat4 view_matrix, double current_time) {
    ...
    glm::mat4 scaling_matrix = glm::scale(glm::mat4(1.0f), glm::vec3(scale_,1.0f));

    // Setup the rotation matrix for the shader
    glm::mat4 rotation_matrix = glm::rotate(glm::mat4(1.0f), glm::radians(GetRotation()), glm::vec3(0.0, 0.0, 1.0));
