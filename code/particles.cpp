...
Particles::Particles(int i) : Geometry()
{
...
void Particles::CreateGeometry(void)
{
    ...
    for (int i = 0; i < NUM_PARTICLES; i++){
        ...
        if (i % 4 == 0){
            ...
            //Have it disperse in a cone (bullet exhaust)
            if (type == 0) {
                theta = (2.0 * (rand() % 10000) / 10000.0f - 1.0f) * 0.2f + pi;
                r = 0.0f + 0.5 * (rand() % 10000) / 10000.0f;
                tmod = (rand() % 10000) / 10000.0f;
            }

			else if (type == 3) {
				theta = (2.0 * (rand() % 10000) / 10000.0f - 1.0f) * 0.0f + pi;
				r = 0.0f + 0.5 * (rand() % 10000) / 10000.0f;
				tmod = (rand() % 10000) / 10000.0f;
			}
            //Have it disperse in a circle
            else if (type == 2) {
                theta = (two_pi * (rand() % 1000) / 1000.0f);
                r = 0.0f + 0.05 * (rand() % 10000) / 10000.0f;
                tmod = (rand() % 10000) / 10000.0f;
            }
            else {
                theta = (two_pi * (rand() % 1000) / 1000.0f);
